...
run_eval() {
    ...
    shift 1
    echo "==> ${tag}"
    python -m gluefactory.eval.endomapper_dense1500 --conf eval_endomapper1500 --tag "${tag}" "$@" "${EXTRA_ARGS[@]}"
}
...
run_eval "05-sift_cudasift+lg_ENDO" \
    ...
    model.matcher.width_confidence=-1 \
    model.matcher.filter_threshold=0.1 \
    checkpoint=/workspace/data/training_outputs/05-sift_cudasift+lg_ENDO_3D/checkpoint_best.tar
...
#txt
python tools/report_summaries.py  --benchmark endomapper_dense1500  --format plain --sort-by rel_pose_error_mAA --descending
#csv
python tools/report_summaries.py  --benchmark endomapper_dense1500  --format csv --sort-by rel_pose_error_mAA --descending
#md
python tools/report_summaries.py  --benchmark endomapper_dense1500  --format md --sort-by rel_pose_error_mAA --descending
#endregion
